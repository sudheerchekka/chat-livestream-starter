...
public class ServerApp {
    ...
    public static void main(String[] args) {
        ...
        // Create an access token using our Twilio credentials
        get("/token1", "application/json", (request, response) -> {
            // Create a Faker instance to generate a random username for the connecting user
            Faker faker = new Faker();
            // Generate a random username for the connecting client
            //String identity = faker.name().firstName() + faker.name().lastName() + faker.address().zipCode();
            String identity = "chatuser1";
         ...
         get("/token2", "application/json", (request, response) -> {
            // Create a Faker instance to generate a random username for the connecting user
            Faker faker = new Faker();
            // Generate a random username for the connecting client
            //String identity = faker.name().firstName() + faker.name().lastName() + faker.address().zipCode();
            String identity = "chatuser2";
        ...
        //TODO: remove hardcoded SIDs
        get("/members", (request, response) -> {

            Twilio.init(configuration.get("TWILIO_API_KEY"),configuration.get("TWILIO_API_SECRET"),configuration.get("TWILIO_ACCOUNT_SID"));
            ResourceSet<Member> members = Member.reader(
                "IS330eb34a69ba4e999e37f039ce40da02",
                "CH8146980f64c44c07b0a242a8a3fe4726")
            .limit(20).read();
            ...
            for(Member record : members) {
                String memberName = record.getIdentity();
                jsonObj = new JsonObject();
                jsonObj.addProperty("member", memberName);
